...
<body>
    <div style="display: flex;align-items: center;justify-content: space-between;margin-bottom: 25px">
        <div style="width: 35%">
            <img src="{{asset('backend/image/settings/'.$siteSettings->logo)}}" style="max-height: 100px; width: 100px;" alt="logo" />
        </div>
        <div style="width: 65%">
    ...
    <div style="display: flex;justify-content: space-between;">
        <div style="width: 33.33%;margin-right: 20px;">
            ...
            <p style="font-size: 18px;font-family: 'Open Sans', sans-serif;color: #000;font-weight: 600;margin-bottom: 0;">
                {{$orders->name}}<br> {{$orders->phone}}<br> {{$orders->address}}
            </p>
        </div>
        ...
        <div style="width: 33.33%;margin-top: 30px;">
            <p style="font-size: 18px; margin-bottom:2px"><b>Order Number: {{$orders->invoice_number}}</b></p>
            <p style="font-size: 18px; margin-bottom:2px"><b>Order Date: {{$orders->created_at->format('d/m/y')}}</b></p>
    ...
    <table class="invoice-table" style="margin-bottom: 0px; border: 1px solid #000;width: 100%;margin-top: 15px;">
        ...
        <tr>        
          <td style="width: 60%">{{$details->product->name}}</td>
            ...
            <td style="width: 20%;">{{$details->price}}</td>
            <td style="width: 20%;">Size: {{$details->size??"N.A"}}</td>
            <td style="width: 20%;">Color: {{$details->color??"N.A"}}</td>          
        </tr>
        @endforeach
        {{-- <tr>
            <td style="width: 60%">Jeans</td>
            <td style="width: 20%">1</td>
            <td style="width: 20%;">1200 Tk.</td>
            <td style="width: 20%;">Size: 32</td>
            <td style="width: 20%;">Color: Black</td>
        </tr> --}}
        <tr>
            <td></td>
            <td><strong>Subtotal</strong></td>
            <td><strong>{{$orders->price - $orders->charge}} Tk.</strong></td>
        </tr>
        {{-- <tr>
            <td></td>
        ...
        <tr>
            ...
            <td><strong>{{$orders->charge}} Tk.</strong></td>
        </tr>
        <tr>
            <td></td>
            <td><strong>Total</strong></td>
            <td><strong>{{$orders->price}} Tk.</strong></td>
        </tr>
    </table>
    <!-- /Header -->
    <hr>
